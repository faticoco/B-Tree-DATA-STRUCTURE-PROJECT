...
public:
    ...
    void insert(T num, string& fileref) {
        if (Root == NULL) {
            ...
            if (ptrCurrent->count < m) {
                ...
                while (num > ptrCurrent->node_key[counter] && counter < ptrCurrent->count) {
                    counter++;
                }

                // sorting all the keys inorder to insert in order
                for (int j = ptrCurrent->count; j > counter; j--)
                {
            ...
            else 
            {
                ...
                for (count = 0; num > extra_key[count] && count < m;) {
                    count++;  // finding index to insert
                }
                // pushing the arrays 
                for (int count_2 = m + 1; count_2 > count; count_2--) {
                    extra_key[count_2] = extra_key[count_2 - 1];
                    extra_filerefs[count_2] = extra_filerefs[count_2 - 1];
                }
    ...
    void deleting_node(T to_delete)
    {
        ...
        {     //sibling_ind[0]
            ...
            else
            {
                ...
                if (sibling_ind[0] >= 0)
                {
                    check_left_for_borrowing(Parent_node, sibling_ind[0], ptrCurrent , min_keys);
                }
                ...
                if (sibling_ind[1] <= Parent_node->count) {
                    check_right_for_borrowing(Parent_node, sibling_ind[1], ptrCurrent ,min_keys, i);
                }




                //**************Initializing left and right siblings of the node to be deleted***************//
                ...
                if (sibling_ind[0] >= 0)
                {
                    check_left_for_deletion(Parent_node, sibling_ind[0], ptrCurrent);
                }

                else if (sibling_ind[1] <= Parent_node->count)
                {
                    check_right_for_deletion(Parent_node , sibling_ind[1] , ptrCurrent );
                }
            }
    ...
    void check_left_for_borrowing(Node<T>* Parent_node , int sibling_ind , Node <T>* ptrCurrent , int min_keys)
    {
        //initialize left sibling
        Node<T>* node_on_left = Parent_node->node_childs[sibling_ind];
        ...
        if (node_on_left->count >= (min_keys + 1)) {
            ...
            Parent_node->node_key[sibling_ind] = ptrCurrent->node_key[0];
            Parent_node->filerefs[sibling_ind] = ptrCurrent->filerefs[0];
    ...
    void check_right_for_borrowing(Node<T>* Parent_node , int sibling_ind , Node <T>* ptrCurrent , int min_keys, int i)
    {
        //Initialize right sibling
        Node<T>* node_on_right = Parent_node->node_childs[sibling_ind];
        ...
        if (node_on_right->count >= min_keys + 1)
        {
            ...
            Parent_node->node_key[sibling_ind - 1] = node_on_right->node_key[0];
            Parent_node->filerefs[sibling_ind - 1] = node_on_right->filerefs[0];
            return;
        }
    }
    ...
    void check_left_for_deletion(Node<T>* Parent_node , int sibling_ind , Node <T>* ptrCurrent)
    {
        Node<T>* node_on_left = Parent_node->node_childs[sibling_ind];
        ...
        //Left node keys added
        node_on_left->count = node_on_left->count + ptrCurrent->count;

        node_on_left->node_childs[node_on_left->count] = ptrCurrent->node_childs[ptrCurrent->count];

        //delete_non_leaf(Parent_node->node_key[ sibling_ind], Parent_node, ptrCurrent);

        delete_all(ptrCurrent);
       
    }

    void delete_all(Node <T>* ptrCurrent)
    {
    ...
    void check_right_for_deletion(Node<T>* Parent_node , int sibling_ind , Node <T>* ptrCurrent)
    {
        ...
        ptrCurrent->count += node_on_right->count;

        ptrCurrent->node_childs[ptrCurrent->count] = node_on_right->node_childs[node_on_right->count];

        cout << "\nMerging nodes" << endl;
        //delete_non_leaf(Parent_node->node_key[ sibling_ind[1] - 1], Parent_node, node_on_right);
         delete_right_node(node_on_right);
    }


    void delete_right_node(Node<T>* node_on_right)
    {
 ...
 /*Displays the record of all names beginning with this string*/
    void searchingBystring(T search_item, Node<T>* ptrCurrent)
    {
        if (Root == NULL) {
            cout << "\n\t\t(!) Tree not populated"
                << endl;
        ...
        else
        {
            ...
            else
            {
                for (int i = 0; i <= ptrCurrent->count; i++)
                {
                    if (ptrCurrent->node_key[i].compare(search_item) == 0)
                    {
...
void queries() {
    ...
    do {
        ...
        cout << "\n\n\n\t(1). Search Record by Specific Name             (2). Search Record by Name Contained. "
            ...
            <<"\n\t(7). Search by gender (M or F)           \t(0).Quit \t\t > ";
